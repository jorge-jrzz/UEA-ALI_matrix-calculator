...
def insertar_matriz():
    try:
        ...
        if fila > 5 or columna > 5:
            raise ValueError(
                "Error, límite superado. Valores fuera del rango.")
        elif fila < 1 or columna < 1:
            raise ValueError("No puedes ingresar datos menores a 1.")
        elif fila != columna:
            raise ValueError(
                "Error, la matriz debe ser cuadrada. Ingresa el mismo número de filas y columnas")
        matriz = np.zeros((fila, columna))

        for i in range(fila):
            for j in range(columna):
                matriz[i, j] = float(
                    input(f"Ingrese el elemento en la posición ({i + 1}, {j + 1}): "))
        print("\nMatriz ingresada:")
        print(matriz)
        return matriz
